...
import { Bot, Video as VideoIcon, RefreshCw, X, Check } from 'lucide-react';
...
// Interface for triggering swipes from the parent
...
const DraggableCard = forwardRef<CardHandle, DraggableCardProps>(({ video, onSwipe }, ref) => {
  ...
  // --- Transformations based on X position ---
  const rotate = useTransform(x, [-200, 200], [-35, 35]);
  const scale = useTransform(x, [-200, 0, 200], [0.85, 1, 0.85]);

  // Opacity for "REAL" vs "AI" stamps
  const opacityRight = useTransform(x, [40, 150], [0, 1]); // Real
  const opacityLeft = useTransform(x, [-40, -150], [0, 1]); // AI

  // Dynamic Background/Shadow Colors
  ...
  const boxShadow = useTransform(
    ...
    [-200, 0, 200],
    [
      "-30px 20px 60px rgba(239, 68, 68, 0.6)", // Red glow left
      "0px 10px 40px rgba(0,0,0,0.5)",          // Neutral
      "30px 20px 60px rgba(34, 197, 94, 0.6)"   // Green glow right
    ]
  );
  ...
  useImperativeHandle(ref, () => ({
    triggerSwipe: async (direction) => {
      if (direction === 'left') {
        await controls.start({ x: -800, rotate: -45, opacity: 0, scale: 0.8, transition: { duration: 0.3 } });
        onSwipe('left');
      } else {
        await controls.start({ x: 800, rotate: 45, opacity: 0, scale: 0.8, transition: { duration: 0.3 } });
        onSwipe('right');
  ...
  return (
    <motion.div
      ...
      >
        {/* Overlay Indicators */}
        <motion.div style={{ opacity: opacityRight }} className="absolute top-10 left-8 z-30 border-4 border-green-500 rounded-2xl p-4 transform -rotate-12 bg-black/20 backdrop-blur-md shadow-[0_0_50px_rgba(34,197,94,0.6)]">
          <span className="text-green-500 font-black text-5xl uppercase tracking-widest drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">REAL</span>
        </motion.div>

        <motion.div style={{ opacity: opacityLeft }} className="absolute top-10 right-8 z-30 border-4 border-red-500 rounded-2xl p-4 transform rotate-12 bg-black/20 backdrop-blur-md shadow-[0_0_50px_rgba(239,68,68,0.6)]">
            <span className="text-red-500 font-black text-5xl uppercase tracking-widest drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">AI</span>
        </motion.div>

        {/* Color Tints */}
        <motion.div style={{ backgroundColor: bgRight }} className="absolute inset-0 z-20 pointer-events-none mix-blend-overlay" />
        <motion.div style={{ backgroundColor: bgLeft }} className="absolute inset-0 z-20 pointer-events-none mix-blend-overlay" />
...
const SwipeDeck: React.FC<SwipeDeckProps> = ({ currentVideo, onSwipe, isLoading }) => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full h-full max-w-md mx-auto px-4 perspective-1000">
      ...
      {/* Visual Hints / Buttons */}
      {/* Increased z-index to 20 to sit above the card */}
      <div className="absolute bottom-6 flex justify-between w-full px-8 z-20">
         ...
         >
            ...
            <span className="text-xs font-bold text-red-400 tracking-widest uppercase drop-shadow-md">AI</span>
         </button>
         ...
         >
            ...
            <span className="text-xs font-bold text-green-400 tracking-widest uppercase drop-shadow-md">REAL</span>
         </button>
      </div>
    </div>
  );
